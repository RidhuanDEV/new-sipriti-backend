...
import { PHASE5_PERMISSION_SEEDS } from "../rbac-source.js";
...
export async function up(queryInterface: QueryInterface): Promise<void> {
  ...
  for (const permission of PHASE5_PERMISSION_SEEDS) {
    ...
    if (userRoleId && ["view_hki", "create_hki", "edit_hki", "delete_hki", "submit_hki", "view_dashboard"].includes(permission.name)) {
      await queryInterface.sequelize.query(
        "INSERT IGNORE INTO role_permissions (role_id, permission_id) VALUES (:roleId, :permissionId)",
