...
import { PHASE6_GRANULAR_PERMISSION_SEEDS } from "../rbac-source.js";
...
export async function up(queryInterface: QueryInterface): Promise<void> {
  ...
  for (const permission of PHASE6_GRANULAR_PERMISSION_SEEDS) {
    const existingPermissionId = await findIdByName(
      queryInterface,
      "permissions",
      permission.name,
    );
